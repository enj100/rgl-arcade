...
const kothSettingsEmbed = require("../commands/king-of-hill/embeds/Settings");
...
const buildChestsSettingsEmbed = require("../commands/chests/embeds/settings");
const ChestItems = require("../commands/chests/models/ChestsItems");
const { createWalletEmbed } = require("../commands/wallet/embeds/wallet");
const buildGoldSwapSettings = require("../commands/swap-gold/embeds/settings");
...
module.exports = {
	...
	async execute(interaction) {
		...
		////////////////// Whip Duel [house] //////////////
		///////////////////////////////////////////////////
		else if (interaction?.customId?.startsWith("dice_house_repeat-")) {
			const userId = interaction.customId.split("-")[1];
			const amount = parseFloat(interaction.customId.split("-")[2]);
		...
		////////////////// Whip Duel [house] //////////////
		///////////////////////////////////////////////////
		else if (interaction?.customId?.startsWith("whip_house_repeat-")) {
			const userId = interaction.customId.split("-")[1];
			const amount = parseFloat(interaction.customId.split("-")[2]);
			...
			if (interaction.user.id !== userId) {
				return await interaction.reply({
					content: `*❗ You cannot repeat this bet for another user!*`,
					ephemeral: true,
				});
			}
		...
		////////////////// Gold Swap //////////////////////
		///////////////////////////////////////////////////
		else if (interaction?.customId?.startsWith("close_swap_gold_ticket-")) {
		...
		} else if (interaction?.customId?.startsWith("swap_to_osrs_open_ticket-")) {
			const userId = interaction?.customId?.split("-")[1];
			if (interaction.user.id !== userId) {
				return await interaction.reply({
					content: `*❗ You cannot swap gold for another user!*`,
					ephemeral: true,
				});
			}

			// get current embed description
			const currentEmbedDescription = interaction.message.embeds[0]?.description.split("\n");
			// get amount to swap from text
			const match = currentEmbedDescription[0].match(/Amount to Swap:\*\* ([\d.]+) RGL-Tokens/);
			const amount = match ? parseFloat(match[1]) : null;

			if (!amount) {
				return await interaction.reply({
					content: `*❗ Invalid amount to swap!*`,
					ephemeral: true,
				});
			}

			const settings = interaction.client.goldSwapSettings;
			if (!settings.category || !settings.logs_channel) {
				return await interaction.reply({
					content: "*❗ Gold swap is not enabled at the moment.*",
					ephemeral: true,
				});
			}

			const rate = await Rate.findOne({ where: { id: 0 } });

			const embed = new EmbedBuilder()
				.setTitle(`🔁 New Tokens Swap Order`)
				.setDescription(
					`\`▸ Customer :\` ${interaction.user}\n\`▸ Swapping :\` ${amount.toFixed(2)} Tokens\n\`▸ Rate     :\` ${
						rate.rate
					} Tokens/M\n\`▸ OSRS GP  :\` ${(amount / rate.rate).toFixed(2)}M\n\n${
						settings.ticket_message || "*✔️ Staff will join the ticket soon!*"
					}`
				)
				.setFooter({ text: "RGL-Arcade Swap to OSRS" });

			if (settings.thumbnail) {
				embed.setThumbnail(settings.thumbnail);
				embed.setFooter({ text: "RGL-Arcade Swap to OSRS", iconURL: settings.thumbnail });
			}
			if (settings.color) {
			...
			const closeTicketButton = new ButtonBuilder()
				.setCustomId("close_swap_gold_ticket-" + interaction.user.id)
				.setLabel("🔒 Close Ticket")
				.setStyle(ButtonStyle.Danger);

			const row = new ActionRowBuilder().addComponents(closeTicketButton);

			const channel = await createTicket(interaction.guild, settings.category, interaction.user, settings.staff_role, embed, [
				row,
			]);
		...
		} else if (interaction?.customId?.startsWith("swap_to_osrs_submit-")) {
			const userId = interaction?.customId?.split("-")[1];
			...
			if (!amount || isNaN(amount) || amount <= 0) {
				return await interaction.reply({
					content: `*❗ Please enter a valid amount to swap!*`,
					ephemeral: true,
				});
			}
			...
			if (user.balance < amount) {
				return await interaction.reply({
					content: `*❗ You do not have enough balance to swap!*`,
					ephemeral: true,
				});
			}
			...
			const embed = new EmbedBuilder(`💵 Swap to OSRS`)
				.setTitle("✅ Confirm Swap")
				.setDescription(
					`**▸ Amount to Swap:** ${amount.toFixed(2)} RGL-Tokens\n**▸ Rate:** ${rate.rate} Tokens/M\n**▸ Estimated OSRS GP:** ${(
						amount / rate.rate
					).toFixed(2)}M\n\n*✅ Click a button bellow to open a ticket.*`
				)
				.setColor("FF0000")
				.setFooter({ text: `RGL-Arcade` });

			if (settings.color) {
				embed.setColor(settings.color);
			}
			if (settings.thumbnail) {
				embed.setThumbnail(settings.thumbnail).setFooter({ text: `RGL-Arcade`, iconURL: settings.thumbnail });
			}

			const openTicketBtn = new ButtonBuilder()
				.setCustomId("swap_to_osrs_open_ticket-" + userId)
		...
		} else if (interaction?.customId?.startsWith("swap_to_osrs-")) {
			const userId = interaction?.customId?.split("-")[1];
			...
			// modal to enter the amount of tokens to swap
			const modal = new ModalBuilder()
				.setCustomId("swap_to_osrs_submit-" + userId)
				.setTitle("Swap to OSRS")
				.addComponents(
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							.setCustomId("amount")
							.setLabel("Tokens Amount")
							.setRequired(true)
							.setMaxLength(7)
		...
		} else if (interaction?.customId === "gold_swap_settings_edit_submit") {
			const settings = interaction.client.goldSwapSettings;

			settings.color = interaction.fields.getTextInputValue("gold_swap_color") || "FF0000";
			settings.thumbnail = interaction.fields.getTextInputValue("gold_swap_thumbnail") || null;
			settings.ticket_message = interaction.fields.getTextInputValue("gold_swap_ticket_message") || null;

			await settings.save();

			interaction.client.goldSwapSettings = settings;

			const { embeds, components } = await buildGoldSwapSettings(interaction);
			await interaction.update({ embeds, components });
		} else if (interaction?.customId === "gold_swap_settings_edit") {
			const settings = interaction.client.goldSwapSettings;

			// create modal to edit color, thumbnail, ticket_message
			const modal = new ModalBuilder()
				.setCustomId("gold_swap_settings_edit_submit")
				.setTitle("Edit Gold Swap Settings")
				.addComponents(
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							.setCustomId("gold_swap_color")
					...
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							.setCustomId("gold_swap_thumbnail")
					...
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							.setCustomId("gold_swap_ticket_message")
							.setLabel("Ticket Message")
							.setRequired(false)
							.setStyle(TextInputStyle.Paragraph)
							.setValue(settings.ticket_message || "")
					)
				);

			await interaction.showModal(modal);
		} else if (interaction?.customId === "gold_swap_admin_role_select") {
			const roleId = interaction.values[0];

			const settings = await GoldSwapSettings.findOne({ where: { id: 0 } });
			if (!settings) return;
			...
			const { embeds, components } = await buildGoldSwapSettings(interaction);
			await interaction.update({ embeds, components });
		} else if (interaction?.customId === "gold_swap_category_select") {
			...
			const { embeds, components } = await buildGoldSwapSettings(interaction);
			await interaction.update({ embeds, components });
		} else if (interaction?.customId === "gold_swap_logs_channel") {
			const channelId = interaction.values[0];

			const settings = await GoldSwapSettings.findOne({ where: { id: 0 } });
			if (!settings) return;

			settings.logs_channel = channelId;
			await settings.save();
			...
			const { embeds, components } = await buildGoldSwapSettings(interaction);
			await interaction.update({ embeds, components });
		} else if (interaction?.customId === "other_settings" && interaction?.values[0] === "swap_gold_settings") {
			const { embeds, components } = await buildGoldSwapSettings(interaction);
		...
		////////////////// Cashback ///////////////////////
		///////////////////////////////////////////////////
		else if (interaction?.customId?.startsWith("claim_cashback-")) {
			const userId = interaction.customId.split("-")[1];
			if (userId !== interaction.user.id) {
				return await interaction.reply({
					content: `*❗ You cannot claim cashback for another user!*`,
					ephemeral: true,
				});
			}
			const [userObj] = await User.findOrCreate({ where: { discord_id: userId }, defaults: { discord_id: userId } });
			if (userObj.cashback < 20) {
				return await interaction.reply({
					content: `*❗ You need at least 20.00 Tokens cashback to be able to claim!*`,
					ephemeral: true,
				});
			}
			userObj.balance += userObj.cashback;
			userObj.cashback = 0;
			await userObj.save();
			...
			const { embed, row } = createWalletEmbed(user, userObj, settings);

			await interaction.update({ embeds: [embed], components: [row] });
			await interaction.followUp({ content: `*✅ You have successfully claimed your cashback!*`, ephemeral: true });
		}
		///////////////////////////////////////////////////
		////////////////// Chests Duel ///////////////////////
		///////////////////////////////////////////////////
		else if (interaction?.customId?.startsWith("chests_decline-")) {
			...
			if (!interaction.user.id.includes(player1) && !interaction.user.id.includes(player2)) {
				return await interaction.reply({
					content: `*❗ You cannot decline this chests duel!*`,
					ephemeral: true,
				});
			}
			...
			const canceledBtn = new ActionRowBuilder().addComponents(
				new ButtonBuilder()
					.setCustomId(`chests_canceled-${player1}-${player2}`)
					.setLabel("❌ Chests Duel Canceled")
					.setStyle(ButtonStyle.Secondary)
					.setDisabled(true)
			);

			await interaction.update({
				content: `*❌ ${interaction.user} has declined the Chests Duel.*`,
				components: [canceledBtn],
				ephemeral: true,
			});
		} else if (interaction?.customId?.startsWith("chests_accept-")) {
			const [rnd, player1, player2, amount] = interaction.customId.split("-");
			...
			if (!interaction.user.id.includes(player2)) {
				return await interaction.reply({
					content: `*❗ You cannot accept this war duel!*`,
					ephemeral: true,
				});
			}
			...
			if (player1Wallet.balance < parseFloat(amount) || player2Wallet.balance < parseFloat(amount)) {
				return await interaction.reply({
					content: `*❗ One of the players does not have enough RGL-Tokens to play this war duel!*`,
					ephemeral: true,
				});
			}

			// prepare 5 chests loot for each player - must be preloaded
			const allItems = await ChestItems.findAll();
			if (allItems.length <= 0) {
				return;
			}

			const embedInteraction = interaction.message.embeds[0];
			const description = embedInteraction.description.split("\n");
			...
			for (let i = 0; i < 5; i++) {
				...
				const randomIndex2 = Math.floor(Math.random() * allItems.length);

				p1.push(allItems[randomIndex]);
				p2.push(allItems[randomIndex2]);

				p1SumNow += allItems[randomIndex].item_value;
				p2SumNow += allItems[randomIndex2].item_value;
			}
			const winner = p1SumNow > p2SumNow ? player1 : p2SumNow > p1SumNow ? player2 : "Tie";
			...
			await processGameResult({
				...
				winner,
				amount,
				player1Score: `${p1SumNow.toLocaleString("en-US")}`,
				player2Score: `${p2SumNow.toLocaleString("en-US")}`,
				gameName: "🧰 Chest Duel",
				interaction,
			});

			const settings = interaction.client.serverSettings;
			const descriptionText = `🟢 **Player 1:** ${userMention(player1)}\n🔴 **Player 2:** ${userMention(
				player2
			)}\n\n▸ **Bet:** ${amount} RGL-Tokens`;
			const embed = new EmbedBuilder()
				.setTitle("🧰 Chests Duel 🧰")
				.setDescription(`*⏳ Please wait...*`)
				.setFooter({
					text: `${settings.brand_name} - Chest Duel`,
			...
			const intervalId = setInterval(async () => {
				try {
					if (counter < 5) {
						p1Loot += `**► ${p1[counter].emoji ? p1[counter].emoji : "-"}${p1[counter].item_name}** | ${p1[
							counter
						].item_value.toLocaleString("en-US")}\n`;
						p1Sum += p1[counter].item_value;
						p2Loot += `**► ${p2[counter].emoji ? p2[counter].emoji : "-"}${p2[counter].item_name}** | ${p2[
							counter
						].item_value.toLocaleString("en-US")}\n`;
						p2Sum += p2[counter].item_value;

						embed.setDescription(
							`${descriptionText}\n\n**${userMention(player1)} loot:**\n${p1Loot}**💰Total:** ${p1Sum.toLocaleString(
								"en-US"
							)}\n\n**${userMention(player2)} loot:**\n${p2Loot}**💰Total:** ${p2Sum.toLocaleString("en-US")}`
						);
						counter++;
					...
					if (counter >= maxCount - 1) {
						let winner = "Tie";
						if (p1Sum > p2Sum) {
							winner = userMention(player1);
						}
						if (p2Sum > p1Sum) {
							winner = userMention(player2);
						}
						embed.setDescription(
							`${descriptionText}\n\n**${userMention(player1)} loot:**\n${p1Loot}**💰Total:** ${p1Sum.toLocaleString(
								"en-US"
							)}\n\n**${userMention(player2)} loot:**\n${p2Loot}**💰Total:** ${p2Sum.toLocaleString(
								"en-US"
							)}\n\n**🏆 WINNER:** ${winner}`
						);
						await interaction.message.edit({ embeds: [embed] });
		...
		} else if (interaction?.customId === "other_settings" && interaction.values[0] === "chests_battle_settings") {
			const { embeds, components } = await buildChestsSettingsEmbed(interaction);
			await interaction.reply({ embeds, components, ephemeral: true });
		} else if (interaction?.customId === "chests_battle_add_item") {
			const items = await ChestItems.count();
			if (items >= 25) {
				return await interaction.reply({ content: "*❗ You cannot add more than 25 items to the chest!*" });
			}

			// create modal with emoji, item_value, item_name
			const modal = new ModalBuilder()
				.setCustomId("chests_battle_add_item_modal")
				.setTitle("Add Item to Chest")
				.addComponents(
					new ActionRowBuilder().addComponents(
						new TextInputBuilder().setCustomId("item_name").setLabel("Item Name").setRequired(true).setStyle(TextInputStyle.Short)
					),
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
			...
			await interaction.showModal(modal);
		} else if (interaction?.customId === "chests_battle_add_item_modal") {
			const itemName = interaction.fields.getTextInputValue("item_name");
			const itemValue = interaction.fields.getTextInputValue("item_value");
			const itemEmoji = interaction.fields.getTextInputValue("item_emoji");

			// Validate item value
			if (isNaN(itemValue) || parseFloat(itemValue) <= 0) {
				return await interaction.reply({
					content: `*❗ Invalid item value specified!*`,
					ephemeral: true,
				});
			}

			// Add item to chest
			await ChestItems.create({
				item_name: itemName,
		...
		} else if (interaction?.customId === "chests_battle_edit_item") {
			const itemId = interaction.values[0];
			const item = await ChestItems.findByPk(itemId);
			...
			// create modal with emoji, item_value, item_name
			const modal = new ModalBuilder()
				.setCustomId(`chests_battle_edit_item_modal-${itemId}`)
				.setTitle("Edit Item in Chest")
				.addComponents(
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
					...
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							...
							.setRequired(true)
							.setStyle(TextInputStyle.Short)
							.setValue(item.item_value.toString())
					),
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							.setCustomId("item_emoji")
			...
			await interaction.showModal(modal);
		} else if (interaction?.customId?.startsWith("chests_battle_edit_item_modal-")) {
			...
			const { embeds, components } = await buildChestsSettingsEmbed(interaction);
			await interaction.update({ embeds, components, ephemeral: true });
		} else if (interaction?.customId?.startsWith("chests_battle_delete_item")) {
			const itemId = interaction.values[0];
			const item = await ChestItems.findByPk(itemId);
		...
		////////////////// War Duel ///////////////////////
		///////////////////////////////////////////////////
		else if (interaction?.customId?.startsWith("war_accept-")) {
			...
			const winner = card1.rank > card2.rank ? player1 : card2.rank > card1.rank ? player2 : "Tie";

			const canvas = createCanvas(70, 110);
			const ctx = canvas.getContext("2d");
			const card1_png = await loadImage(`./commands/war/assets/${card1.value}_of_${card1.suit}.png`);
			ctx.drawImage(card1_png, 0, 0, 70, 110);
			const buffer = canvas.toBuffer("image/png");
			const file1 = new AttachmentBuilder(buffer, { name: "player1.png" });

			const canvas2 = createCanvas(70, 110);
			const ctx2 = canvas2.getContext("2d");
			const card2_png = await loadImage(`./commands/war/assets/${card2.value}_of_${card2.suit}.png`);
			ctx2.drawImage(card2_png, 0, 0, 70, 110);
			const buffer2 = canvas2.toBuffer("image/png");
			const file2 = new AttachmentBuilder(buffer2, { name: "player2.png" });

			const embed1 = new EmbedBuilder().setImage("attachment://player1.png").setColor("FF0000");
			const embed2 = new EmbedBuilder().setImage("attachment://player2.png").setColor("FF0000");

			const kothSettings = await KothSettings.findOne({ where: { id: 0 } });
			if (kothSettings && kothSettings.status && winner !== "Tie") {
				// increment in the leaderboard
				const [entry] = await KothLeaderboard.findOrCreate({
					where: { discord_id: winner },
					defaults: { discord_id: winner, wins: 0 },
				});
				await KothLeaderboard.increment("wins", { by: 1, where: { discord_id: winner } });
			}
			...
			await processGameResult({
				...
				winner,
				amount,
				player1Score: `${card1.value} of ${card1.suit}`,
				player2Score: `${card2.value} of ${card2.suit}`,
				gameName: "🃏 War Duel",
				interaction,
			});
			...
			setTimeout(async () => {
				await interaction.followUp({
					content: `🃏 ${userMention(player1)} drew... `,
					embeds: [embed1],
					files: [file1],
				});
			}, 1000);
			setTimeout(async () => {
				await interaction.followUp({
					content: `🃏 ${userMention(player2)} drew... `,
					embeds: [embed2],
					files: [file2],
				});
			}, 2000);
			setTimeout(async () => {
				await interaction.followUp({
					content: `${winner === "Tie" ? "🃏 It's a tie!" : "🃏 The winner is <@" + winner + ">! 🥳"}`,
				});
			}, 3000);
		} else if (interaction?.customId?.startsWith("war_decline-")) {
			const [action, player1, player2] = interaction.customId.split("-");
			if (!interaction.user.id.includes(player1) && !interaction.user.id.includes(player2)) {
				return await interaction.reply({
					content: `*❗ You cannot decline this war duel!*`,
					ephemeral: true,
				});
			}
			...
			const canceledBtn = new ActionRowBuilder().addComponents(
				new ButtonBuilder()
					.setCustomId(`war_canceled-${player1}-${player2}`)
					.setLabel("❌ War Duel Canceled")
					.setStyle(ButtonStyle.Secondary)
					.setDisabled(true)
			);

			await interaction.update({
				content: `*❌ ${interaction.user} has declined the War Duel.*`,
				components: [canceledBtn],
				ephemeral: true,
			});
		}
		...
		////////////////// Koth ///////////////////////////
		///////////////////////////////////////////////////
		else if (interaction?.customId === "other_settings" && interaction?.values[0] === "koth_settings") {
			const { embeds, components } = await kothSettingsEmbed(interaction);

			await interaction.reply({ embeds, components, ephemeral: true });
		} else if (interaction?.customId === "koth_settings_edit") {
			const settings = await KothSettings.findOne({ where: { id: 0 } });
			const modal = new ModalBuilder().setCustomId("koth_settings_edit_submit").setTitle("Edit Koth Settings");

			const winnersAmountInput = new TextInputBuilder()
				.setCustomId("koth_winners_amount")
				.setLabel("Winners Amount (1-5)")
				.setStyle(TextInputStyle.Short)
				.setValue(settings.winners_amount.toString())
				.setRequired(true);

			const thumbnailInput = new TextInputBuilder()
				.setCustomId("koth_thumbnail")
				.setLabel("Thumbnail URL")
				.setStyle(TextInputStyle.Short)
				.setValue(settings.thumbnail || "")
				.setRequired(false);
			...
			modal.addComponents(
				new ActionRowBuilder().addComponents(winnersAmountInput),
				new ActionRowBuilder().addComponents(thumbnailInput),
				new ActionRowBuilder().addComponents(colorInput)
			);
			await interaction.showModal(modal);
		} else if (interaction?.customId === "koth_settings_edit_submit") {
			const winnersAmount = parseInt(interaction.fields.getTextInputValue("koth_winners_amount"));
			const thumbnail = interaction.fields.getTextInputValue("koth_thumbnail");
			const color = interaction.fields.getTextInputValue("koth_color");

			if (isNaN(winnersAmount) || winnersAmount < 1 || winnersAmount > 5) {
				return await interaction.reply({
					content: "❗ Winners amount must be a number between 1 and 5.",
					ephemeral: true,
				});
			}

			const settings = await KothSettings.findOne({ where: { id: 0 } });
			settings.winners_amount = winnersAmount;
		...
		} else if (interaction?.customId === "koth_settings_auto_run") {
			const settings = await KothSettings.findOne({ where: { id: 0 } });
		...
		} else if (interaction?.customId === "koth_prize_edit") {
			const prizeId = interaction.values[0];
			const [prize] = await KothPrize.findOrCreate({ where: { place: prizeId }, defaults: { place: prizeId } });

			const modal = new ModalBuilder().setCustomId(`koth_prize_edit_submit-${prizeId}`).setTitle(`Edit ${prizeId}st Place Prize`);

			const prizeNameInput = new TextInputBuilder()
				.setCustomId("koth_prize_name")
				.setLabel("Prize Name")
				.setStyle(TextInputStyle.Short)
				.setValue(prize.prize_name || "")
				.setRequired(true);

			const prizeTokensInput = new TextInputBuilder()
				.setCustomId("koth_prize_tokens")
				.setLabel("Prize Tokens")
				.setStyle(TextInputStyle.Short)
				.setValue(prize.prize_tokens ? prize.prize_tokens.toString() : "0")
				.setRequired(true);

			modal.addComponents(
				new ActionRowBuilder().addComponents(prizeNameInput),
				new ActionRowBuilder().addComponents(prizeTokensInput)
			);

			await interaction.showModal(modal);
		} else if (interaction?.customId?.startsWith("koth_prize_edit_submit-")) {
			const prizeId = interaction.customId.split("-")[1];
			const prizeName = interaction.fields.getTextInputValue("koth_prize_name");
			const prizeTokens = parseInt(interaction.fields.getTextInputValue("koth_prize_tokens"));

			if (isNaN(prizeTokens) || prizeTokens < 0) {
				return await interaction.reply({
					content: "❗ Prize tokens must be a positive number.",
					ephemeral: true,
				});
			}
			...
			prize.prize_tokens = prizeTokens;
			await prize.save();
		...
		} else if (interaction?.customId === "koth_channel_select") {
			const channelId = interaction.values[0];
			const settings = await KothSettings.findOne({ where: { id: 0 } });
		...
		} else if (interaction?.customId === "koth_settings_status") {
			const settings = await KothSettings.findOne({ where: { id: 0 } });
			...
			if (settings.status) {
				...
				if (channel) {
					...
					let prizesString = prizes
						...
						.join("\n");
					const { files, components, filename } = await buildKothLeaderboard(interaction.client, 0, true);
					const embed = new EmbedBuilder()
				...
				await channel.send({ content: "*❌ Koth Race has been stopped.*" });
			}
			...
			await interaction.followUp({
				content: `📢 Koth race has been ${settings.status ? "__started__" : "__stopped__"}.`,
				ephemeral: true,
			});
		} else if (interaction?.customId === "koth_settings_draw_winners") {
			const settings = await KothSettings.findOne({ where: { id: 0 } });
			if (!settings.status) {
				return await interaction.reply({ content: "*❌ Koth Race is not currently active.*", ephemeral: true });
			}
		...
		///////////////////////////////////////////////////
			...
			await interaction.reply({ embeds, components, ephemeral: true });
		} else if (interaction?.customId === "fp_live_reset_profit") {
		...
		///////////////////////////////////////////////////
			...
			const cashbackInput = new TextInputBuilder()
				.setCustomId("subs_cashback_percentage")
				.setLabel("Cashback Percentage")
				.setStyle(TextInputStyle.Short)
				.setValue(settings?.cashback_percentage?.toString() || "0.0")
				.setRequired(true)
				.setMaxLength(10);
			...
			const row2 = new ActionRowBuilder().addComponents(cashbackInput);
			...
			const cashbackPercentage = interaction.fields.getTextInputValue("subs_cashback_percentage");
			...
			if (isNaN(cashback) || cashback < 0) {
				return await interaction.reply({
					content: "❗ Invalid cashback percentage! Please enter a valid number greater than or equal to 0.",
					ephemeral: true,
				});
			}
			...
			settings.cashback_percentage = cashback;
		...
		///////////////////////////////////////////////////
			...
			if (!interaction.user.id.includes(player2)) {
				return await interaction.reply({
					content: `*❗ You cannot accept this coin flip duel!*`,
					ephemeral: true,
				});
			}
		...
		///////////////////////////////////////////////////
			...
			interaction?.values[0] === "guess_game_settings"
		) {
		...
		///////////////////////////////////////////////////
			...
			interaction?.customId === "other_settings" &&
			interaction?.values[0] &&
			interaction?.values[0] === "jackpot_settings"
		) {
		...
		} else if (interaction?.customId === "jackpot_announce_channel_select") {
			const channel = interaction.values[0];
			const [settings] = await JackpotSettings.findOrCreate({
				where: { id: 0 },
				defaults: { id: 0 },
			...
			settings.announce_channel = channel || null;
			await settings.save();
			const { embeds, components } = await buildJackpotSettingsEmbed(interaction);
			await interaction.update({
				embeds,
			...
			if (settings.status) {
				settings.next_draw_date = new Date(Date.now() + 4 * 60 * 60 * 1000); // next draw in 4 hours
		...
		///////////////////////////////////////////////////
			...
			interaction?.customId === "giveaways_create" &&
			interaction?.values[0] &&
			interaction?.values[0] === "change_info"
		) {
			const [giveaway] = await Giveaway.findOrCreate({
				where: { status: "creating" },
			});
