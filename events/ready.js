...
async function fetchInitialData(client) {
	...
	await Rate.findOrCreate({ where: { id: 0 }, defaults: { id: 0, rate: 0.18 } });
	[client.goldSwapSettings] = await GoldSwapSettings.findOrCreate({
		where: { id: 0 },
		defaults: {
			id: 0,
		},
		...
		cron.schedule("*/1 * * * *", async () => {
			try {
				...
				if (settings.status) {
					...
					const profit = totalPot - totalPayout;
				...
				console.error("Error occurred while processing Live Flower Poker:", error);
			}
		});
