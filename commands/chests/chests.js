	...
	async execute(interaction) {
		if (interaction) {
			...
			buttons.addComponents(
				new ButtonBuilder()
					.setCustomId(`chests_accept-${player1}-${player2}-${amount}`)
					.setLabel("✔️")
					.setStyle(ButtonStyle.Success)
			);
			buttons.addComponents(
				new ButtonBuilder().setCustomId(`chests_decline-${player1}-${player2}`).setLabel("✖️").setStyle(ButtonStyle.Danger)
			);
			...
			setTimeout(async () => {
				try {
					...
					console.error("Error editing chests duel message:", e);
				}
			}, 60000);
		}
	},
