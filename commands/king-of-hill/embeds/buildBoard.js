...
async function generateKothImage(users = null, client) {
	...
	if (users[1]) {
		...
		if (userObj) {
			...
			context.fillText(`${users[1].wins} Win(s)`, 2870, 1650);
		}
	}
	if (users[2]) {
		...
		if (userObj) {
			...
			context.fillText(`${users[2].wins} Win(s)`, 2070, 1810);
		}
	}
	if (users[3]) {
		...
		if (userObj) {
			...
			context.fillText(`${users[3].wins} Win(s)`, 1280, 1990);
		}
	}
	if (users[4]) {
		...
		if (userObj) {
			...
			context.fillText(`${users[4].wins} Win(s)`, 500, 2090);
		}
	}
