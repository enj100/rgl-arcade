	...
	async execute(interaction) {
		if (interaction) {
			...
			const embed = new EmbedBuilder()
				...
				.setFooter({
					text: `${settings.brand_name} - Coin Flip Duel`,
			...
			buttons.addComponents(
				new ButtonBuilder()
					.setCustomId(`coin_flip_accept-${player1}-${player2}-${amount}`)
					.setLabel("✔️")
					.setStyle(ButtonStyle.Success)
			);
			buttons.addComponents(
				new ButtonBuilder().setCustomId(`coin_flip_decline-${player1}-${player2}`).setLabel("✖️").setStyle(ButtonStyle.Danger)
			);
			...
			setTimeout(async () => {
				try {
					...
					console.error("Error editing coin flip duel message:", e);
				}
			}, 60000);
		}
	},
