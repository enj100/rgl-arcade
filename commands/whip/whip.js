	...
	async execute(interaction) {
		if (interaction) {
			...
			const embed = new EmbedBuilder()
				.setTitle("⚔️ Whip Duel ⚔️")
				.setDescription(
					`🟢 **Player 1 (left):** ${userMention(player1)}\n🔴 **Player 2 (right):** ${userMention(
						player2
					)}\n\n▸ **Bet:** ${amount.toFixed(2)} RGL-Tokens`
				)
				.setFooter({
					text: `${settings.brand_name} - Whip Duel`,
			...
			buttons.addComponents(
				new ButtonBuilder()
					.setCustomId(`whip_accept-${player1}-${player2}-${amount}`)
					.setLabel("✔️")
					.setStyle(ButtonStyle.Success)
			);
			buttons.addComponents(
				new ButtonBuilder().setCustomId(`whip_decline-${player1}-${player2}`).setLabel("✖️").setStyle(ButtonStyle.Danger)
			);
			...
			setTimeout(async () => {
				try {
					...
					console.error("Error editing whip duel message:", e);
				}
			}, 60000);
		}
	},
