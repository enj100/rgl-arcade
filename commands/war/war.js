    ...
    async execute(interaction) {
        if (interaction) {
            ...
            const embed = new EmbedBuilder()
                ...
                .setFooter({
                    text: `${settings.brand_name} - War Duel`,
            ...
            buttons.addComponents(
                new ButtonBuilder().setCustomId(`war_accept-${player1}-${player2}-${amount}`).setLabel("✔️").setStyle(ButtonStyle.Success)
            );
            buttons.addComponents(
                new ButtonBuilder().setCustomId(`war_decline-${player1}-${player2}`).setLabel("✖️").setStyle(ButtonStyle.Danger)
            );
            ...
            setTimeout(async () => {
                try {
                    ...
                    console.error("Error editing war duel message:", e);
                }
            }, 60000);
        }
    },
